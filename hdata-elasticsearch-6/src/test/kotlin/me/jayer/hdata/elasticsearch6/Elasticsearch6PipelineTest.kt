...
class Elasticsearch6PipelineTest {
    ...
    @Test
    fun `a full pipeline write with no failures produces no dead letters, every row reaches ES`() {
    ...
    @Test
    fun `a full pipeline sends every row to the dead letter on a whole-batch failure, carrying the original row`() {
