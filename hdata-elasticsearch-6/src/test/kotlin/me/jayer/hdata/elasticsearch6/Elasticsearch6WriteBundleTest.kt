...
class Elasticsearch6WriteBundleTest {
    ...
    @Test
    fun `submits once batch_size is reached, sending the whole batch at once`() {
    ...
    @Test
    fun `finishBundle flushes the remaining rows short of a full batch`() {
    ...
    @Test
    fun `only the failed rows go to the dead letter, the rest are written normally`() {
    ...
    @Test
    fun `a whole-batch connection failure sends every row to the dead letter with the real exception`() {
        val fn = writeFn(batchSize = 2)
    ...
    @Test
    fun `a write failure throws directly when the dead letter is not enabled`() {
    ...
    @Test
    fun `with no schema_fields configured, it writes the document column, matching what the read side produces`() {
