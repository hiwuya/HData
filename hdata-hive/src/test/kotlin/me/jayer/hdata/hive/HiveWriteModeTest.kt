...
class HiveWriteModeTest {
    ...
    @Test
    fun `append write - a second job does not overwrite the first job's file`() {
        TestHive().use { hive ->
    ...
    @Test
    fun `overwrite write - only the last write's data remains`() {
        TestHive().use { hive ->
            partitionedTable(hive)
            write(hive, listOf(1L to "2024-01-01", 2L to "2024-01-01"))
    ...
    @Test
    fun `overwrite write only touches the partitions written this time, the rest are untouched`() {
        TestHive().use { hive ->
            partitionedTable(hive)
    ...
    @Test
    fun `dynamic partitions - one job writes several partitions by row value and registers all of them`() {
        TestHive().use { hive ->
    ...
    @Test
    fun `dynamic partitions register a new partition without erroring on an existing one`() {
        TestHive().use { hive ->
            partitionedTable(hive)
    ...
    @Test
    fun `overwrite write is also supported for an unpartitioned table`() {
        TestHive().use { hive ->
    ...
    @Test
    fun `append writes produce distinct file names`() {
        TestHive().use { hive ->
            partitionedTable(hive)
    ...
    @Test
    fun `a wrong write_mode errors at graph construction time`() {
