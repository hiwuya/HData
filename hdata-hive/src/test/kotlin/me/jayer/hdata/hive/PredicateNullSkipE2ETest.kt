 ...
 * ORC/Parquet API, concentrating the nulls at the start and the end of the file so that some units are entirely NULL-free
 * or entirely NULL, deterministically triggering the skip.
 *
 * @author wuya
 */
class PredicateNullSkipE2ETest {
    ...
    @Test
    fun `ORC IS NULL skips a stripe with no null values at all`() {
    ...
    @Test
    fun `Parquet IS NULL skips a row group with no null values at all`() {
        TestHive().use { hive ->
            runAndAssertSkip(
                hive,
                "t_isnull_parquet",
    ...
    @Test
    fun `ORC IS NOT NULL falls back to row-level filtering only (ORC column statistics have no null count, so a whole-null block cannot be proven)`() {
    ...
    @Test
    fun `Parquet IS NOT NULL skips a row group that is entirely null`() {
        TestHive().use { hive ->
            runAndAssertSkip(
                hive,
                "t_isnotnull_parquet",
