...
class PredicateEvaluatorTest {
    ...
    @Test
    fun `matches IS NULL and IS NOT NULL`() {
    ...
    @Test
    fun `matches ANDs multiple predicates`() {
    ...
    @Test
    fun `canSkip skips decimal GT when the block's max is not large enough`() {
    ...
    @Test
    fun `canSkip skips GT when the block's max is not large enough`() {
        val p = HivePredicate("id", PredicateOp.GT, idType, NumericValue(BigDecimal(100)))
    ...
    @Test
    fun `canSkip skips EQ when the block's range excludes the value`() {
    ...
    @Test
    fun `canSkip compares strings by unsigned byte order`() {
    ...
    @Test
    fun `canSkip skips IS NULL when the block has no null`() {
    ...
    @Test
    fun `canSkip skips IS NOT NULL when the whole block is null`() {
    ...
    @Test
    fun `canSkip does not skip when no statistics are available`() {
        val p = HivePredicate("id", PredicateOp.GT, idType, NumericValue(BigDecimal(100)))
        assertFalse(PredicateEvaluator.canSkip(listOf(p), emptyMap()))
