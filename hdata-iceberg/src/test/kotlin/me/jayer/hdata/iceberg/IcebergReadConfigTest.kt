...
class IcebergReadConfigTest {
    ...
    @Test
    fun `catalog name and field names are strictly validated`() {
    ...
    @Test
    fun `split_size defaults to 128MB and must be positive`() {
        val config = IcebergReadConfig("/wh", table = "db.t", schemaFields = listOf("id:INT64"))
    ...
    @Test
    fun `filter and limit config validation`() {
        val config = IcebergReadConfig("/wh", table = "db.t", schemaFields = listOf("id:INT64"))
    ...
    @Test
    fun `aggregation supports count_min_max_sum_avg`() {
    ...
    @Test
    fun `aggregates and limit are mutually exclusive, and output column names must not repeat`() {
        val config = IcebergReadConfig("/wh", table = "db.t")
    ...
    @Test
    fun `limit mode rejects a split_size that would not take effect`() {
