...
object JdbcTypeRegistry {
    ...
    init {
        ...
        registerJdbcType(Byte::class, FieldTypes.BYTE) { rs, i -> rs.getByte(i) }
        registerJdbcType(Short::class, FieldTypes.INT16) { rs, i -> rs.getShort(i) }
        registerJdbcType(Int::class, FieldTypes.INT32) { rs, i -> rs.getInt(i) }
        registerJdbcType(Long::class, FieldTypes.INT64) { rs, i -> rs.getLong(i) }
        registerJdbcType(Float::class, FieldTypes.FLOAT) { rs, i -> rs.getFloat(i) }
        registerJdbcType(Double::class, FieldTypes.DOUBLE) { rs, i -> rs.getDouble(i) }
        registerJdbcType(arrayOf(BigInteger::class, BigDecimal::class), FieldTypes.DECIMAL) { rs, i ->
            rs.getBigDecimal(i)
        }
        registerJdbcType(arrayOf(Date::class, LocalDate::class), FieldTypes.DATE) { rs, i ->
            rs.getDate(i)?.toLocalDate()
        }
        registerJdbcType(arrayOf(Time::class, LocalTime::class), FieldTypes.TIME) { rs, i ->
            rs.getTime(i)?.toLocalTime()
        }
        registerJdbcType(LocalDateTime::class, FieldTypes.DATETIME) { rs, i -> rs.getTimestamp(i)?.toLocalDateTime() }
        registerJdbcType(Timestamp::class, FieldTypes.TIMESTAMP) { rs, i -> rs.getTimestamp(i)?.toInstant() }
        ...
        // String type
        registerJdbcType(String::class, FieldTypes.STRING) { meta: JdbcColumnMeta ->
        ...
        // byte[] type
        registerJdbcType(ByteArray::class, FieldTypes.BYTES) { meta: JdbcColumnMeta ->
            ResultSetGetter { rs, i ->
                when (meta.type) {
    ...
    fun registerJdbcType(
        ...
        typeProviders.add(object : TypeProvider {
    ...
    fun getFieldType(columnMeta: JdbcColumnMeta): Schema.FieldType? {
        return if (columnMeta.type == JDBCType.ARRAY) {
            val elementJdbcType = getFieldType(columnMeta.copy(type = JDBCType.valueOf(columnMeta.typeName)))
    ...
    fun getResultSetGetter(columnMeta: JdbcColumnMeta): ResultSetGetter? {
        return if (columnMeta.type == JDBCType.ARRAY) {
            val elementResultSetGetter =
                getResultSetGetter(columnMeta.copy(type = JDBCType.valueOf(columnMeta.typeName)))
    ...
    fun getPreparedStatementSetter(type: Schema.FieldType): PreparedStatementSetter {
        return if (type.typeName == Schema.TypeName.ARRAY) {
            ...
            val elementFieldValueConverter = getFieldValueConverter(elementType)
            ...
            PreparedStatementSetter { ps, row, i ->
                ...
                ps.setObject(i + 1, if (value != null) getFieldValueConverter(type).convert(value) else null)
            }
        }
    }
}