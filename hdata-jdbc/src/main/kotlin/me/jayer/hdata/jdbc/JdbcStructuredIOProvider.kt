...
import me.jayer.hdata.core.spi.StructuredIOProvider
import me.jayer.hdata.core.spi.StructuredSink
import me.jayer.hdata.core.spi.StructuredSource
import me.jayer.hdata.core.util.ObjectMappers
import java.util.*
...
class JdbcStructuredIOProvider : StructuredIOProvider {
    ...
    override fun createSource(config: Map<String, Any>): StructuredSource {
        val jdbcSourceDescriptor = ObjectMappers.getTomlObjectMapper().convertValue(config, me.jayer.hdata.jdbc.JdbcSourceDescriptor::class.java)
        updateDataSourceConfig(jdbcSourceDescriptor.dataSourceConfig, config)
        jdbcSourceDescriptor.validate()
        return me.jayer.hdata.jdbc.JdbcStructuredSource(jdbcSourceDescriptor)
    }

    override fun createSink(config: Map<String, Any>): StructuredSink {
        val jdbcSinkDescriptor = ObjectMappers.getTomlObjectMapper().convertValue(config, me.jayer.hdata.jdbc.JdbcSinkDescriptor::class.java)
        updateDataSourceConfig(jdbcSinkDescriptor.dataSourceConfig, config)
        jdbcSinkDescriptor.validate()
        return me.jayer.hdata.jdbc.JdbcStructuredSink(jdbcSinkDescriptor)
    }
}