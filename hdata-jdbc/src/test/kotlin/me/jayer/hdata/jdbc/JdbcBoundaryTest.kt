...
class JdbcBoundaryTest {
    ...
    @Test
    fun `RowMapper throws when field count and reader count disagree`() {
    ...
    @Test
    fun `toSchema throws outright for a query with no columns`() {
    ...
    @Test
    fun `resolve returns null when the array element type isn't supported`() {
    ...
    @Test
    fun `toSchema throws an actionable exception when the array element type isn't supported`() {
        val column = JdbcColumn(
            label = "ids",
            type = JDBCType.ARRAY,
            typeName = "UUID ARRAY",
    ...
    @Test
    fun `read config binding throws on an unknown property`() {
    ...
    @Test
    fun `the partitioned-read DoFn can be serialized and shipped`() {
